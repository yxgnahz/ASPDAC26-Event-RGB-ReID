import copy
import sys
sys.path.append('/mnt/home/zxwang22/code/Event-ReID/models/mitml')
import torch
...
class embed_net(nn.Module):
    ...
    def forward(self, x1, x2, adj, modal=0, seq_len = 8, cpa = False):
        ...
        x_l = self.avgpool(x_t).squeeze() # (2 * bs * seq_len, hid_dim)
        x_l = x_l.view(x_l.size(0)//t, t, -1).permute(1, 0, 2) # (seq_len, 2 * bs, hid_dim)

        x_h = self.avgpool(x).squeeze()
        x_h = x_h.view(x_h.size(0)//t, t, -1).permute(1, 0, 2) # (seq_len, 2 * bs, hid_dim)

        h0 = torch.zeros(2, x_l.shape[1], x_l.shape[2]).cuda() # (2, 2 * bs, hid_dim)
