...
class embed_net(nn.Module):
    def __init__(self, class_num, drop=0.2, arch="resnet50", with_data_aug=False, with_graph_feature=False,\
                 with_frame_id_loss=False, with_square_avg=False, with_attack=False):
        ...
        self.with_graph_feature = with_graph_feature
        self.with_frame_id_loss = with_frame_id_loss
        self.with_square_avg = with_square_avg
        self.with_attack = with_attack
        print("#" * 30)
        print(f'with data aug: {self.with_data_aug}')
        print(f'with graph feature: {self.with_graph_feature}')
        print(f'with frame id loss" {self.with_frame_id_loss}')
        print(f'with square avg: {self.with_square_avg}')
        print(f'with attack: {self.with_attack}')
        print("#" * 30)



    def forward(self, x1, x2, modal=0, seq_len=6, edges=None):
        b, c, h, w = x1.size() # (num_identity*num_pos, seq_len*3, h, w)
        t = seq_len
        x1 = x1.view(int(b * seq_len), int(c / seq_len), h, w) #(num_identity*num_pos*seq_len, 3, h, w)
        ...
        # exit()
        # style augmentation
        if self.training and self.with_data_aug:
            # IR modality
            # frame_batch = seq_len*16
            """
            Modified here to adapt the batch size. In the original paper, they sample 8 identities with 2 videos (RGB or IR) for each.
            """
            frame_batch = seq_len*b
            delta = torch.rand(frame_batch) + 0.5*torch.ones(frame_batch) # [0.5-1.5]
            inter_map = delta.unsqueeze(dim=1).unsqueeze(dim=1).unsqueeze(dim=1).cuda() # (seq_len * num_identity * num_pos, 1, 1, 1)
            x2 = x2 * inter_map
        ...
        if self.training:
            ...
            # print(f'x local shape: {x_local.shape}')
            x_local_2 = rearrange(x_local, '(b t) n->b t n', t=seq_len) # (2 * num_id * num_pos, seq_len, hid_dim)
            # print(f'x local 2 shape: {x_local_2.shape}')
            ...
            if self.with_square_avg:
                p = 3.0
                x_global = (torch.mean(x_local_2**p, dim=1) + 1e-12)**(1/p)
            else:
                x_global = torch.mean(x_local_2, dim=1)
    ...
    def intra_interact(self, x_gcn, edges, seq_len):
        ...
        for k in range(0, x_gcn.shape[0], 2):  # [num_id*num_pos, seq_len, 2048]
            x_gcn_tmp_1 = x_gcn[k]
            x_gcn_tmp_2 = x_gcn[k+1]
            x_gcn_tmp = torch.cat((x_gcn_tmp_1, x_gcn_tmp_2), dim=0)
            edge_weights_1 = rearrange(torch.cosine_similarity(x_gcn_tmp_1.unsqueeze(dim=-1), x_gcn_tmp_2.permute(1,0).unsqueeze(0), dim=1), 'm n ->(m n)') # (6 ,6) -> 36
            edge_weights_2 = rearrange(torch.cosine_similarity(x_gcn_tmp_2.unsqueeze(dim=-1), x_gcn_tmp_1.permute(1,0).unsqueeze(0), dim=1), 'm n ->(m n)') # (6, 6) -> 36
    ...
    def inter_interact(self, x_gcn, edges, seq_len):
        bs, _ = x_gcn.shape
        num_identity = int(bs/(2 * seq_len * 2)) # by default num_pos = 2
        x_v_gcn = x_gcn[0:seq_len*2*num_identity, :]
        x_i_gcn = x_gcn[seq_len*2*num_identity:, :]
        x_v_gcn_feat_list = []
        x_i_gcn_feat_list = []
