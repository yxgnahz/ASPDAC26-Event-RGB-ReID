...
def fine_grained_frame_distance_cosine(feat_1, feat_2, logit_scale=None, topk=1, topk_smallest=False):
    """
    use the difference instead of cosine similarity
    input:
        feat_1 shape (N1, seq_len, hid_dim)
    ...
    def get_logits(f1, f2):
        ...
        dense_feat_1 = f1.reshape(-1, k)
        dense_feat_2 = f2.reshape(-1, k)
        dist = torch.matmul(dense_feat_1, dense_feat_2.t())
        # # Compute pairwise distance
        # dist_1 = torch.pow(dense_feat_1, 2).sum(dim=1, keepdim=True).expand(i*j, l*m)
        # dist_2 = torch.pow(dense_feat_2, 2).sum(dim=1, keepdim=True).expand(l*m, i*j)
        # dist = dist_1 + dist_2.t()
        ...
        #
        # logits_1 = scale * dist
        logits = dist.reshape(i, j, l, m).permute(0, 2, 1, 3) #(i, l, j, m)
        return logits
...
def fine_grained_id_set_distance(track_dist, num_pos=2, topk=1):
    """
    Input:
        track_dist shape: (n_id * num_pos, n_id * num_pos)
    Output:
        id_dist shape: (n_id, n_id)
    """
    n, n = track_dist.shape
    n_id = n // num_pos
    track_dist = track_dist.reshape(n_id, num_pos, n_id, num_pos).permute(0, 2, 1, 3) #(n_id, n_id, num_pos, num_pos)
    id_dist_largest = 0.5 * (track_dist.topk(topk, dim=-1, largest=True)[0].mean(dim=(2, 3)) +
                                track_dist.topk(topk, dim=-2, largest=True)[0].mean(dim=(2, 3)))
    id_dist_smallest = 0.5 * (track_dist.topk(topk, dim=-1, largest=False)[0].mean(dim=(2, 3)) +
                                track_dist.topk(topk, dim=-2, largest=False)[0].mean(dim=(2, 3)))
    return id_dist_largest, id_dist_smallest
