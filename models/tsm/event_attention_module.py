...
class EventIntegralModule(nn.Module):
    def __init__(self, arch='resnet50', seq_len=6):
        ...
        self.event_conv = nn.Conv2d(self.conv1.out_channels * seq_len, self.conv1.out_channels, kernel_size=1, stride=1,
                                    padding=0)
    ...
    def forward(self, x):
        ...
        integral_feat = integral_feat.reshape(-1, integral_feat.shape[2], integral_feat.shape[3],
                                              integral_feat.shape[4])  # (B*T, T*64, H, W)
        ...
        integral_feat = integral_feat.reshape(B, -1, integral_feat.shape[1], integral_feat.shape[2],
                                              integral_feat.shape[3])  # (B, T, convlayer1_hid_dim, H, W)
