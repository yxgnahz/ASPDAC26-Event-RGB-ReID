...
__all__ = ['TSMResNet', 'tsmresnet50', 'tsmresnet101']

model_urls = {
        'resnet18': 'https://download.pytorch.org/models/resnet18-5c106cde.pth',
        'resnet34': 'https://download.pytorch.org/models/resnet34-333f7ec4.pth',
        'tsmresnet50': 'http://data.lip6.fr/cadene/pretrainedmodels/resnet50-19c8e357.pth',
        'tsmresnet101': 'http://data.lip6.fr/cadene/pretrainedmodels/resnet101-5d3b4d8f.pth'
}
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, num_segments, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False) # TDN: bias = True
        self.bn1 = nn.BatchNorm2d(planes)

        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False) # TDN: bias = True
        self.bn2 = nn.BatchNorm2d(planes)

        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1, bias=False) # TDN: bias = True
...
class TSMResNet(nn.Module):
    ...
    def __init__(self, num_segments, block, layers, num_classes=1000, arch = 'resnet50'):
        self.inplanes = 64
        self.num_segments = num_segments
        super(TSMResNet, self).__init__()

        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
                                bias=False) # TDN: bias = True
        ...
        self.layer4 = self._make_layer(self.num_segments,block, 512, layers[3], stride=1)
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False), # TDN: bias = True
...
def tsmresnet18(num_segments=8,pretrained=False,num_classes=1000):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TSMResNet(num_segments,BasicBlockShift, [2, 2, 2, 2], num_classes=num_classes, arch = 'resnet18')
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']),strict=False)
    return model


def tsmresnet34(num_segments=8,pretrained=False,num_classes=1000):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TSMResNet(num_segments,BasicBlockShift, [3, 4, 6, 3], num_classes=num_classes, arch = 'resnet34')
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34']),strict=False)
    return model

def tsmresnet50(num_segments=8,pretrained=False,num_classes=1000):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TSMResNet(num_segments,BottleneckShift, [3, 4, 6, 3], num_classes=num_classes, arch = 'resnet50')
    if pretrained:
         model.load_state_dict(model_zoo.load_url(model_urls['tsmresnet50']),strict=False)
    return model


def tsmresnet101(num_segments,pretrained=False,num_classes=1000):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = TSMResNet(num_segments,BottleneckShift, [3, 4, 23, 3], num_classes=num_classes)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['tsmresnet101']),strict=False)
    return model
