...
class embed_net(nn.Module):
    def __init__(self, class_num, arch='resnet50', frame_bn=False, inference_norm=True):
    ...
    def forward(self, x1, x2, modal=0, seq_len=8, frame_id_loss=False, return_raw_frame=False, gem_pooling=False):
        b, c, h, w = x1.size()
        t = seq_len
        ...
        x = self.base_resnet(x) # (num_id*num_pos*2*seq_len, c, h, w)
        _, c, h, w = x.shape
        x_frame_patch = x.view(-1, t, c, h, w) #(num_id*num_pos*2, seq_len, c, h, w)
        x_pool = self.avgpool(x).squeeze() # (num_id*num_pos*2*seq_len, c)
        x_frame_pool = x_pool.view(-1, t, c) #(num_id*2*num_pos, seq_len, c)
        x_pool = x_pool.view(x_pool.size(0)//t, t, -1) # (num_id*num_pos*2, seq_len, hid_dim)
        ...
        if self.frame_bn:
            x_frame_patch = x_frame_patch.view(-1, c, h, w)
            x_frame_patch = self.bottleneck2(x_frame_patch)
            x_frame_patch = x_frame_patch.view(-1, t, c, h, w)
        ...
        if self.training:
            if frame_id_loss:
                return x_pool, x_frame_patch, x_frame_return, self.logit_scale_dense, self.classifier(feat), \
                       self.local_classifier(x_frame_pool_feat.reshape(-1, c))
            else:
                return x_pool, x_frame_patch, x_frame_pool, self.logit_scale_dense, self.classifier(feat)
        else:
            if self.inference_norm:
                x_frame_patch = x_frame_patch / x_frame_patch.norm(dim=2, keepdim=True)
                x_frame_pool = x_frame_pool / x_frame_pool.norm(dim=-1, keepdim=True)
            return self.l2norm(feat), x_frame_patch, self.l2norm(x_frame_pool_feat)
